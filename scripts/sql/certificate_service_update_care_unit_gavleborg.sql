...
BEGIN
    ...
    INSERT INTO organizationProvider VALUES
        ...
        ('SE2321000198-019363', 'Primärvård Ljusdal',               'SE2321000198-021163', 'Barnavårdscentral Färila - Los Din hälsocentral S',        'SE2321000198-054390', 'VO Färila - Los Din hälsocentral',             'SE2321000198-054434', 'Färila - Los Din hälsocentral'),
    ...
    -- Add the new care provider in the unit table
    INSERT INTO unit (hsa_id, name, unit_type_key, version)
    SELECT updatedCareProviderId, updatedCareProviderName, 1, 0
        WHERE NOT EXISTS (
        SELECT 1
        FROM unit u
    ...
    SELECT
        ...
        CASE
            ...
            ELSE 'Not Found'
            END AS unit_status,
        COUNT(c.certificate_id) AS certificates_migrated
    FROM organizationProvider op
             LEFT JOIN unit new_subunit ON new_subunit.hsa_id = op.updatedSubunitId
             LEFT JOIN certificate c ON c.issued_on_unit_key = new_subunit.`key`
    GROUP BY
